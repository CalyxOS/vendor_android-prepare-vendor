...
def parse_args():
    ...
    parser.add_argument('-c', '--carrierlist', help='AOSP carrier_list.pb folder',
                        required=True)
    ...
    parser.add_argument('--apn', action='store_true',
                        help='Generate apns-conf.xml')
    parser.add_argument('--vendor', action='store_true',
                        help='Generate vendor.xml')
...
def main():
    ...
    if args.apn is None and args.vendor is None:
        print('Argument --apn or --vendor is required')
        sys.exit(1)
    ...
    with open(os.path.join(carrier_list_folder, 'carrier_list.pb'), 'rb') as pb:
