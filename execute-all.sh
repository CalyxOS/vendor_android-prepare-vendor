...
CARRIERCONFIG_VENDOR_OVERLAY_PATH="$OUT_BASE/vendor_overlay/carrier-naked/packages/apps/CarrierConfig/res/values"
mkdir -p "$CARRIERCONFIG_VENDOR_OVERLAY_PATH"
$EXTRACT_CARRIER_SETTINGS_SCRIPT --carrierlist "$aospCarrierListFolder" \
    --input "$carrierSettingsFolder" \
    --apns "$PRODUCT_R_ROOT/etc/" --vendor "$CARRIERCONFIG_VENDOR_OVERLAY_PATH" || {
